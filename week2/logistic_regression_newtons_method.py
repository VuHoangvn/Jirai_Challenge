...
# a generalization of the one-dimensional normal
# distribution to higher dimentions. Such a distribution is
# specified by its mean and covariance matrix.
# so we generate values input - (x, v, z) using normal distributions

# A probability distribution is a function that provides us the probabilities
# of all possible outcomes of a stochastic process.

# ltes keep x and z closely related (height and weight)
x, z = np.random.multivariate_normal([0,0], [[var_x,r], [r,var_z]],n).T
# blood pressure
v = np.random.normal(0,var_v, n)**3

# create a pandas dataframe (esoly parseable object for manipulation)
...
# A binomial random variable is the number of successes x has in n repeated
# trials of a binomial experiment.
# The probability distribution of a binomial random variable is called 
# a binomial disreibution.
A['y'] = [np.random.binomial(1,p) for p in A.log_odds]

# create a datafrane that encompasses our input data, model formula, and outputs
...
def newton_step(curr, X, lam=None):
	'''One native step of Newton's Method'''
	...
	# create probability matrix, minimium 2 dimensions, tranpose (flip it)
...
def check_coefs_convergence(beta_old, beta_new, tol, iters):
	...
	# calculate the change in te coefficients
	coef_change = np.abs(beta_old - beta_new)

	# if change hasn't reached the threshold and we have more iteratons to go,
	# keep training
	return not (np.any(coef_change>tol) & (iters < max_iter))

## initial conditions
# initital coefficients (weight values), 2 copies, we'll update one
...
while not coefs_converged:
	...
	beta_old = beta
	# perform a single step of newton's optimization on our data,
	# set our dated beta values
	beta = newton_step(beta, X, lam=lam)
	# increment the number of iterations
	iter_count += 1

	# check dor convergence between our old and new beta values
