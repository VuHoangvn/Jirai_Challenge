...
def compute_total_error(m,b): # Computes total mean squares error 
    totalError = 0
    for i in range(len(points)):
        x = points[i, 0]
        y = points[i, 1]
        totalError += (y - (m*x + b)) ** 2 # Error is calculated as y' = mx + b
        #(Assuming linear regression)

    return totalError/float(len(points)) # Returnong the mean squared error
...
def compute_jacobian(point_pair, h = 1e-5): # computes the jacobian of the function total_error
    ...
    for i in range(n):
        ...
        # calculate derivative using first principle method f'(x) = lt(h->0) (f(x+h)-f(x))/h
...
def compute_hessian(point_pair, h = 1e-5):
    # computes the hessian of the function total_error,
    # it is found as the derivative of the jacobian
    n = len(point_pair)
...
def compute_newton(init_points, max_iter = 10000, e = 1e-5):
    # calculate roots of the equation, i.e. find x if f(x) = 0.
    # In our case we want to find the minima point, so we find f'(x) = 0
    point_pair_arr = np.zeros((max_iter, len(init_points))) # initialize m,b values
    ...
    for i in range(max_iter):
        ...
        # calculate the new m, new b using newton's equation x(t+1) = x(t) - f(x(t))/f'(x(t))
        # but we want to find root of f'(x) so we would do x(t+1) = x(t) - f'(x(t))/f''(x(t))
...
def main(): # amin driver function
    init_points = np.array([0.0,1.0]) # initial points
